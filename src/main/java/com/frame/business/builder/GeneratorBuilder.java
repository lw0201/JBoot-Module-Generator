...
public class GeneratorBuilder {
	...
	private final static Logger logger = LoggerFactory
			.getLogger(GeneratorBuilder.class);
	...
	public void generatorEntity(TableInfo tableInfo) {
		...
		try {
			if (null != tableInfo.getPackageName()
					&& !"".equals(tableInfo.getPackageName().trim())) {
				String path = tableInfo.getPackageName().replace(".", "/");
				File file = new File(
						Constant.java + File.separator + path + "/entity");
				if (!file.exists()) {
					file.mkdirs();
				}
				write = new FileWriter(new File(file,
						StringUtils.toUpperCaseFirst(tableInfo.getEntityName())
								+ "VO.java"));
	...
	public void generatorDao(TableInfo tableInfo) {
		...
		try {
			if (null != tableInfo.getPackageName()
				...
				String path = tableInfo.getPackageName().replace(".", "/");
				File file = new File(
						Constant.java + File.separator + path + "/dao");
				if (!file.exists()) {
					file.mkdirs();
				}
				write = new FileWriter(
						new File(file,
								"I" + StringUtils.toUpperCaseFirst(
	...
	public void generatorIService(TableInfo tableInfo) {
		...
		try {
			if (null != tableInfo.getPackageName()
				...
				String path = tableInfo.getPackageName().replace(".", "/");
				File file = new File(
						Constant.java + File.separator + path + "/service");
				if (!file.exists()) {
					file.mkdirs();
				}
				write = new FileWriter(
	...
	public void generatorService(TableInfo tableInfo) {
		...
		try {
			if (null != tableInfo.getPackageName()
					&& !"".equals(tableInfo.getPackageName().trim())) {
				String path = tableInfo.getPackageName().replace(".", "/");
				File file = new File(Constant.java + File.separator + path
						+ "/service/impl");
				if (!file.exists()) {
					file.mkdirs();
				}
	...
	public void generatorController(TableInfo tableInfo) {
		...
		try {
			if (null != tableInfo.getPackageName()
				...
				String path = tableInfo.getPackageName().replace(".", "/");
				File file = new File(
						Constant.java + File.separator + path + "/controller");
				if (!file.exists()) {
					file.mkdirs();
				}
				write = new FileWriter(new File(file,
						StringUtils.toUpperCaseFirst(tableInfo.getEntityName())
								+ "Controller.java"));
				Template template = FreemarkerUtil
